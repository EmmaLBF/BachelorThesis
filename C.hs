...
lambdaLift [] (DefFun tret ifun params body ret) = lambdaLift params body ++ [DefFun tret ifun params body ret]
lambdaLift [CParam i t] (DefFun tret ifun params body ret) = lambdaLift (params ++ [CParam i t]) body ++ [DefFun tret ifun (params ++ [CParam i t]) body ret]
lambdaLift l (Seq x y) =
    let x' = lambdaLift l x
        y' = lambdaLift l y
    in x' ++ y'
lambdaLift [] _ = []
lambdaLift _ _ = []
...
showCFunDecs :: [CStatement] -> String
showCFunDecs [] = ""
showCFunDecs [DefFun prox ifun params _ _] =
    "\n" ++ showProx (typeRep prox) ++ " v" ++ show ifun ++ "(" ++ showCParams params ++ ");"
showCFunDecs (i:is) = showCFunDecs[i] ++ showCFunDecs is

showCFunDefs :: [CStatement] -> String
showCFunDefs [] = ""
showCFunDefs [DefFun prox ifun params body res] = 
    "\n" ++ showProx (typeRep prox) ++ " v" ++ show ifun ++ "(" ++ showCParams params ++ ") {"
    ++ showCStmt 1 body
    ++ "\n" ++ "  return " ++ showCExpression res ++ ";"
    ++ "\n}\n"
showCFunDefs (i:is) = showCFunDefs[i] ++ showCFunDefs is
...
main = do
    ...
    let funDecs = "\n// function declarations" ++ showCFunDecs defs ++ "\n"
    let funDefs = "\n// function definitions" ++ showCFunDefs defs
    let body = "\nint main (void) {" ++ showCStmt 1 c
    let ret = "\n  return " ++ CL.showCExpression (CL.result cl) ++ ";\n}"
    let content  = imports ++ types ++ funDecs ++ funDefs ++ body ++ ret
