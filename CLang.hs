...
data CStatement where
    ...
    While :: CExpression Bool -> CStatement -> CStatement -- condition + body
    Bind :: CStatement -> Int -> CStatement -> CStatement -- give a name to outcome of a to use in 
    Seq :: CStatement -> CStatement -> CStatement -- Do two things in succesion, but do not give intermediate outcome a name, specialization of bind
    DefVar :: (Typeable a) => Int -> CExpression a -> CStatement -- Define a variable
    UpdateVar :: (Typeable a) => Int -> CExpression a -> CStatement -- Update variable 
    DefFun :: Int -> CStatement -> CStatement -- define a function of 1 parameter, use the Int as the name of the parameter

translateExpr :: forall a. Int -> NL.NamedLang a -> (CExpression a, Int)
translateExpr c (NL.Var x) = (Var x, c)
translateExpr c (NL.LInt i) = (Val (IntV i), c)
translateExpr c (NL.LBool b) = (Val (BoolV b), c)
translateExpr c (NL.LIntOp op x y) =    let (x', c') = translateExpr c x
                                            (y', c'') = translateExpr c' y
                                        in (LIntOp op x' y', c'')
translateExpr c (NL.LCmpOp op x y) =    let (x', c') = translateExpr c x
                                            (y', c'') = translateExpr c' y
                                        in (LCmpOp op x' y', c'')
translateExpr c (NL.Prod a b) = let (a', c') = translateExpr c a
                                    (b', c'') = translateExpr c' b
                                in (Prod a' b', c'')
translateExpr c (NL.Fst p) =    let (p', c') = translateExpr c p
                                in (Fst p', c')
translateExpr c (NL.Snd p) = let (p', c') = translateExpr c p
                            in (Snd p', c')
translateExpr c (NL.Apply f x) =    let (f', c') = translateExpr c f
                                        (x', c'') = translateExpr c' x
                                    in (CallExpr f' x', c'')
translateExpr _ _ = error "Not an expression"


translateStmt :: forall a. Int -> NL.NamedLang a -> (CStatement, Int)
translateStmt c (NL.If cond thn els) =  let (cond', c') = translateExpr c cond
                                            (thn', c'') = translateStmt c' thn
                                            (els', c''') = translateStmt c'' els
                                        in (If cond' thn' els', c''')
translateStmt c (NL.Lam i body) = let (body', _) = translateStmt i body
                                  in (DefFun i body', c)
translateStmt c (NL.Fix (NL.Lam i body)) =
    let (body', c') = translateStmt i body
        funDef = DefFun i body'
    in (Bind funDef c' (Return (CallExpr (Var c') (Var c'))), c' + 1)
translateStmt c x = let (x', c') = translateExpr c x
                    in (Return x', c')
...
showCStmt indent (UpdateVar i x) m = "v" ++ show i ++ " =~ " ++ showCExpression indent x m
showCStmt indent (If cond t f) m = 
    "\n" ++ indentStr indent ++ "if " ++ showCExpression indent cond m ++ " {\n"
  ++ indentStr (indent + 1) ++ showCStmt (indent + 1) t m ++ "\n"
  ++ indentStr indent ++ "} else {\n"
  ++ indentStr (indent + 1) ++ showCStmt (indent + 1) f m ++ "\n"
  ++ indentStr indent ++ "}"
showCStmt indent (While cond body) m =
    "\n" ++ indentStr indent ++ "while " ++ showCExpression indent cond m ++ " {\n"
  ++ indentStr (indent + 1) ++ showCStmt (indent + 1) body m ++ "\n"
  ++ indentStr indent ++ "}"
showCStmt indent (Bind x i y) m = showCStmt indent x m ++ "\n" ++ showCStmt indent y m
...
showCStmt indent (DefFun i f) m = 
  ...
  ++ indentStr indent ++ "}"
showCStmt indent (Return x) m = "[Return " ++ showCExpression indent x m ++ "]"
showCStmt indent (DefVar i f) m = "def v" ++ show i ++ " = " ++ showCExpression indent f m
...
main = do
    let (nl, c') = NL.translate 0 AL.fac
        (cl, _) = translateStmt c' nl
        -- (ev, c'') = eval cl Map.empty
    print c'
    putStrLn "--- Translating CL ---"
    putStrLn $ showCStmt 0 cl Map.empty
    -- putStrLn $ show (ev 5)
